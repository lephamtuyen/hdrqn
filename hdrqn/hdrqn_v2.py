...
class MetaController():
    def __init__(self, h_size, rnn_cell, num_of_goals, myScope):
        ...
        with tf.variable_scope(myScope):
            ...
            layer1 = slim.fully_connected(self.scalarInput,
                                          200,
            ...
            layer3 = slim.fully_connected(layer2,
                                          h_size,
                                          activation_fn=None,
                                          normalizer_fn=None,
                                          normalizer_params=None,
            ...
            self.trainLength = tf.placeholder(dtype=tf.int32)
            self.batch_size = tf.placeholder(dtype=tf.int32, shape=[])
            self.convFlat = tf.reshape(slim.flatten(layer3), [self.batch_size, self.trainLength, h_size])
            self.state_in = rnn_cell.zero_state(self.batch_size, tf.float32)
...
def train():
    ...
    path = "./hdrqnv2"  # The path to save our model to.
    ...
    for runs in range(1):
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_episodes):
                meta_e = (
                endE + max(0., (startE - endE) * (float(num_episodes) - max(0., float(i))) / float(num_episodes)))
                ...
                while not d and j < max_epLength:
                    ...
                    if np.random.rand(1) < meta_e or total_steps < pre_train_steps:
                        ...
                        next_goal_idx, meta_interal_state = sess.run([metaController.predict, metaController.rnn_state], \
                                                                     feed_dict={
                                                                         metaController.scalarInput: [initial_state], \
                                                                         metaController.trainLength: 1, \
                                                                         metaController.state_in: meta_interal_state, \
                                                                         metaController.batch_size: 1})
                    ...
                    sub_interal_state = (
                    np.zeros([1, h_size]), np.zeros([1, h_size]))  # Reset the recurrent layer's hidden state
                    while not d and j < max_epLength and goal_reached == False:
                        ...
                        subEpisodeBuffer.append(
                            np.reshape(np.array([s, a, intrinsic_reward, s1, intrinsic_done, goal]), [1, 6]))
                        total_list.append(total)
                        partial_list.append(s1P)
                        ...
                        if total_steps > pre_train_steps:
                            ...
                            if total_steps % (update_freq) == 0:
                                ...
                                sub_state_train = (
                                np.zeros([sub_batch_size, h_size]), np.zeros([sub_batch_size, h_size]))
                                ...
                                if (len(metaBuffer.buffer) > meta_batch_size):
                                    # Reset the recurrent layer's hidden state
                                    meta_state_train = (
                                    np.zeros([meta_batch_size, h_size]), np.zeros([meta_batch_size, h_size]))
                    ...
                    last_internal_state = sub_interal_state[0][0]
                    metaEpisodeBuffer.append(
                        np.reshape(np.array([initial_state, goal_idx, extrinsic_reward, last_internal_state, d]),
                                   [1, 5]))
