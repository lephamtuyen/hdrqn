...
def processState(state1, num_of_state):
    return np.reshape(state1, [num_of_state]) / 255.0
...
def saveAnimation(path, i, full_images, partial_images, time_per_step, size):
    images = np.zeros([len(full_images), size*2+2, size*3+4,3])
    for iter in range(full_images.shape[0]):
        images[iter, 1:size * 2 + 1, 1:size * 2 + 1, :] = cv2.resize(full_images[iter], (size * 2, size * 2))
        images[iter, size/2:3*size/2, size * 2 + 3:size * 3 + 3, :] = cv2.resize(partial_images[iter], (size, size))

    make_gif(images, path + '/frames/total' + str(i) + '.gif', duration=len(images) * time_per_step,
